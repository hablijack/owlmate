...
esp32-s3-sense/lib/Eyes/Eyes.cpp. Everything on the OrangePi side is a mirror, and
...
ORANGEPI_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
REPO_ROOT = os.path.dirname(ORANGEPI_ROOT)
EYES_CPP = os.path.join(REPO_ROOT, "esp32-s3-sense", "lib", "Eyes", "Eyes.cpp")
GENERATOR = os.path.join(ORANGEPI_ROOT, "tools", "gen_expressions.py")
CONFIG_YAML = os.path.join(ORANGEPI_ROOT, "config.yaml")

# The firmware tree is absent from a deployed OrangePi (setup.sh installs only
# orangepi-brain/), so the firmware-comparing tests skip rather than fail there.
...
class TestGeneratedFileIsCurrent(unittest.TestCase):
    ...
    def test_generator_check_passes(self):
        ...
        r = subprocess.run([sys.executable, GENERATOR, "--check"],
                           cwd=ORANGEPI_ROOT, capture_output=True, text=True)
        self.assertEqual(
            r.returncode, 0,
