"""
Robot Owl OrangePi Brain - Supervisor
...
detection). The OrangePi no longer runs its own behavior state machine.

The OrangePi is a supervisor: it logs telemetry and can send high-level policy
...
class Supervisor:
    ...
    def __init__(self, serial: SerialHandler, config: dict, audio=None):
        ...
        # self-wakes on a face or a vibration); the OrangePi adds "wake on speech".
        ...
        # Navigation ("guide me home"): the OrangePi computes a compass bearing to a
    ...
    def register_activity(self, now: float = None) -> None:
        """Record that the user just interacted (the OrangePi heard them speak).
    ...
    def check_auto_sleep(self, now: float = None) -> None:
        ...
        iterations). It is a pure policy decision on the OrangePi side: when the owl
    ...
    def play_sound(self, sound: str) -> bool:
        """Play a sound effect on the OrangePi (MAX98357A I2S amp).

        Audio is entirely on the OrangePi side — the amp is wired to the OrangePi's I2S
