"""
Protocol tests: the ESP32 -> RPi telemetry parser and message dispatcher.
...
class TestParseTelemetryScalars(unittest.TestCase):
    ...
    def test_timestamp_is_arrival_time_not_from_the_wire(self):
        # The frame carries no timestamp; the RPi stamps it on arrival, and
...
class TestParseTelemetryPresenceFlags(unittest.TestCase):
    ...
    The firmware emits each section only while in that state, so the RPi infers
