...
the EyeExpression enum). Everything on the RPi side is a mirror.
...
The generated file is COMMITTED, so a Raspberry Pi deployment never needs the
...
static_asserts against its EyeExpression enum. This file exists so the RPi does
...
def main():
    ...
    if args.check:
        ...
        if current != wanted:
            ...
            return 1
        print(f"{os.path.relpath(OUT_PY, RPI_ROOT)} is up to date")
        return 0

    with open(OUT_PY, "w", encoding="utf-8") as fh:
        fh.write(wanted)
    names = parse_names()
    print(f"wrote {os.path.relpath(OUT_PY, RPI_ROOT)}: "
